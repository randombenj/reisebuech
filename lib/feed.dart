        ...
        builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
          ...
          List<Marker> mapMarker = locations.map((latlong.LatLng loc) {
            return Marker(
                ...
                point: loc,
                builder: (ctx) =>
                    Container(child: Icon(Icons.place, color: Colors.red)));
          }).toList();
          ...
          content.addAll(
                      ...
                      children: [
                        Text(a['title'],
                            style: TextStyle(
                                ...
                                color: widget.color)),
                        Text(a['text'], style: TextStyle(fontSize: 20.0))
                      ]);
                } else {
                  content = Text(a['text'], style: TextStyle(fontSize: 20.0));
                }
                break;
              case 'image':
                images.add(a['file']);
            ...
            return Padding(
                padding: EdgeInsets.all(8.0), child: content // Text(a['type'])
                );
          }).toList());
